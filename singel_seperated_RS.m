...
%         vec = Movie_mat(i, :);
    ...
    rand_seperator = rand_seperator';
    % ON <=> OFF.
    rand_seperator = double(~rand_seperator);
...
for k = 1:full_size + 1
    if k == full_size + 1
        Screen('FillRect', window, whiteColor, inverse_cells(:, (rand_seperator > 0))); 
        Screen('Flip', window);
        Screen('FillRect', window, blackColor);
        Screen('Flip', window);
        KbStrokeWait;
    ...
    % seperate masks by constant 128 ON pixels.
    Screen('FillRect', window, whiteColor, inverse_cells(:, (rand_seperator > 0))); 
    Screen('Flip', window);
